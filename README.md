...
<p align="center">Latest published · <a href="https://github.com/mintgao/vibe-kit/releases/tag/v0.10.1"><code>v0.10.1 Pre-release</code></a></p>
...
> Adopt Vibe Kit from https://github.com/mintgao/vibe-kit/releases/tag/v0.10.1 into the current project. Use this exact trusted version, preserve existing application files and project documentation, complete the health check and evidence-backed project onboarding, then continue with this goal: [my development goal].

### Create a new project with Vibe Kit

Describe the application, target directory, and preferred stack in the same request.

> Create a Next.js expense-tracking app in `./my-app` with the official framework generator. Then adopt Vibe Kit from https://github.com/mintgao/vibe-kit/releases/tag/v0.10.1 into the generated project, verify the installation, establish project context, and continue implementing the first usable version.
...
The normative human and machine contracts are [AGENT_INSTALL.md](AGENT_INSTALL.md) and [agent-install.json](agent-install.json). The 0.10.1 Pre-release uses core/Codex protocol 7 and Hermes adapter protocol 1, Agent-install schema/protocol 4, takeover schema 2, maintenance bridge schema 2, publication schema 5 with a separate closeout schema 3, CLI result schema 2, release-manifest schema 2, transaction/commit schemas 1, and feedback protocol 2. Historical releases remain unchanged. The CLI requires Python 3.9 or later and uses only the standard library.

In 0.10.1 both Agent-install contracts are installed framework-managed files, recorded in the project manifest and included in activation identity. `bin/vibe validate-takeover --format json` authenticates the installed contract/registry and checks the complete closed takeover structure from stdin without persisting or echoing it. A valid result is structural evidence only; it never authenticates a host receipt or claims readiness.

`bin/vibe init`, `adopt` and `upgrade` take a comma-separated `--host` selection over the declared hosts (default `codex`). The installed copy records that selection in the manifest and in the contract's activation identity, and an install carries only the selected hosts' payload; the release payload itself always carries every declared host's files. A recorded selection that no longer matches the installed payload fails closed.
...
The 0.10.1 Pre-release has one narrow compatibility exception for an exact healthy
...
The v0.7 target also recognizes only the audited v0.2 fixture and exact official
v0.3/v0.4 pre-onboarding identities for a create-only persisted `pending` bridge.
Existing valid onboarding is byte-preserved; missing v0.5/v0.6 onboarding is state
loss and blocks before writes. Upgrade is a recoverable transaction. A handled
failure either proves rollback, asks for explicit `recover-upgrade`, or reports
unclassifiable state without overwriting it.

Final installation entries use capability-probed, fd-relative lossless
publication: hard-link no-clobber for absent leaves, atomic exchange for existing
leaves, and an adjacent prepared no-replace directory unit when the first managed
parent is absent. Unsupported primitives block; ordinary rename/replace is not a
fallback for final installation entries.
...
`publication-plan`/`validate-publication` for the exact five-asset GitHub
...
Release validation checks archive safety, checksums, versions, Agent-contract/compiled-registry identity, activation v2 independent recomputation, bounded production-validator cases, and drift across the direct Release, Plugin payload, and expanded marketplace. See the [0.10.1 release notes](docs/releases/0.10.1.md), [post-upgrade takeover decision](docs/decisions/0009-post-upgrade-takeover.md), and [reproducible release decision](docs/decisions/0004-reproducible-release-contract.md).
...
- `v0.10.1` is the selected Pre-release version; public completion is recorded separately. There is no stable Release, public Plugin Directory entry, automatic network updater, or publisher signature and provenance attestation.
