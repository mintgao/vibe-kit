...
- [x] AC-1: 仓库包含用户确认的 MIT LICENSE，Git/Plugin/README/release metadata 使用 `mintgao` 与 `https://github.com/mintgao/vibe-kit`，不保留占位发布身份。
- [x] AC-2: prerelease package 记录非空 commit source ref、clean tree 和 `prerelease` 状态；validator 接受已知状态并拒绝未知状态。
- [x] AC-3: GitHub 附件包含 deterministic distribution bundle；解压后具备 release ZIP、Plugin ZIP、expanded marketplace、manifest 与 SHA256SUMS，可独立完成 `validate-release`。
- [x] AC-4: 全量测试、Python 3.9、官方 Plugin/Skill validator、bundle rebuild parity 与独立 QA 全部通过。
- [x] AC-5: 初始 commit 不包含 `.vibe/local/feedback`、dist、secret、缓存或本机凭据；commit author 与用户确认一致。
- [x] AC-6: `mintgao/vibe-kit` 被创建为 Public，`main` 推送成功，annotated `v0.3.0` tag 指向验收 commit。
- [x] AC-7: GitHub `v0.3.0` Release 标记 Pre-release，标题、notes 与所有附件完整且远端摘要/大小可核对。
- [x] AC-8: 从 GitHub 下载完整 bundle 后在隔离目录完成解包、`validate-release`、`plan init`、`init` 和项目内 `doctor`。
- [x] AC-9: `.vibe/core/feedback.json` 指向 `mintgao/vibe-kit`，但仍保持逐 payload 授权、无自动 Issue/遥测。
- [x] AC-10: 发布证据记录真实 URL、commit/tag、附件摘要、跳过项和 rollback；发布后的 Vibe Kit gap 复盘按 feedback flow 执行。
