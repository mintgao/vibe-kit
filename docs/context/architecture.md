...
- Scenario tests exercise initialization, adoption, planning read-onlyness, collision preflight, successful/conflicting upgrade, configured verification, feedback privacy/idempotency, deterministic packaging, release/Plugin installation, and artifact tamper/drift rejection in temporary directories.
...
- v0.3 upgrades require a trusted local checkout, validated GitHub release payload, or bundled Plugin payload; there is no remote updater or Plugin Directory publication.
- The YAML reader intentionally supports only the generated `commands` mapping, not arbitrary YAML.
...
- `.vibe/core/feedback.json` is managed policy and an optional GitHub destination; the canonical repository is intentionally empty until a publisher target is chosen.
...
- Each candidate separates immutable sanitized report content from mutable occurrence, dismissal, remote-check, and submission state.
- Fingerprints omit project identity and timestamps. GitHub submission binds repository, title, body and labels into a review hash and checks the remote fingerprint immediately before create.
...
The Plugin exposes only `vibe-bootstrap` and `vibe-maintain`; project runtime Skills appear only after installation. The installed repository remains the runtime source of truth. Current outputs are local unpublished candidates; see ADRs 0002, 0004 and 0005.
