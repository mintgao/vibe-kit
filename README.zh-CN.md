...
这套工作方式会跟随项目仓库。协作者换到另一台电脑后，仍能获得同一版本的指令和项目上下文。目前经过验证的 Agent 运行环境是 Codex。Hermes 已作为一等声明宿主发布：拥有自己的能力声明、payload 选择与角色映射，记录为「已支持、未完整验证」。其他 coding Agent 需要单独的适配器和一致性证据，才能获得同样的兼容性声明。
