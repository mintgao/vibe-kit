...
Codex 最终应当说明版本与来源、安装健康度、新规则激活、项目上下文、已配置检查和下一步。0.9.0 契约定义了同任务重载和自动后继任务回执，但当前 Codex/Plugin 没有正向宿主证据，因此仅保证一个兜底动作：在同一项目中新建 Codex 任务。源任务应预填或提供一句可复制的原目标；新任务不需要再确认升级或执行内部命令。
