...
3. Run focused tests and default `./bin/vibe verify . --format json` when configured. A partial `--only` receipt cannot close post-upgrade takeover. Treat configured failed/skipped checks as blocking, report unconfigured checks explicitly, and do not claim a check was run if the environment prevented it.
